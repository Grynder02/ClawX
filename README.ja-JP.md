...
モダンなチャット体験を通じてAIエージェントとコミュニケーションできます。複数の会話コンテキスト、メッセージ履歴、Markdownによるリッチコンテンツレンダリング（GitHub 風テーブルや KaTeX による LaTeX 数式 `$インライン$`、`$$ブロック$$`、`\(インライン\)`、`\[ブロック\]` を含む）に加え、メインコンポーザーの先頭で `@Agent` / `/Skill` チップを使い、マルチエージェントとスキルのワークフローを扱えます。
`@Agent` チップはメンション文字列を送信せず別エージェントへルーティングし、`/Skill` チップは先頭の `/skill-name` プレフィックスとして送信されます。Skill チップをクリックすると右側のプレビュー欄でその Skill の `SKILL.md` を開けます。
