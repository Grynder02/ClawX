...
test.describe('ClawX chat workspace context', () => {
  test('bound session shows read-only workspace and workspace tree uses the same cwd', async ({ launchElectronApp }) => {
    ...
    try {
      ...
      const workspaceGroup = sidebar.locator('[data-testid^="workspace-session-group-"]')
        .filter({ hasText: SESSION_WORKSPACE_LABEL })
      ...
      await expect(workspaceHeader).toContainText(SESSION_WORKSPACE_LABEL);
      await expect(workspaceHeader).toHaveAttribute('title', `Main Agent / ${SESSION_WORKSPACE}`);
  ...
  test('new unbound chat appears under the selected global workspace group', async ({ launchElectronApp }) => {
    const app = await launchElectronApp({ skipSetup: true });
    const nowMs = Date.now();
    ...
    try {
      await installWorkspaceMocks(app, {
        ...
        sessionHistory: [{ role: 'user', content: 'Seeded workspace conversation', timestamp: nowMs }],
      });
      ...
      const sidebar = page.getByTestId('sidebar');
      const globalWorkspaceGroup = sidebar.locator('[data-testid^="workspace-session-group-"]')
        .filter({ hasText: GLOBAL_WORKSPACE_LABEL })
        .filter({ hasText: /agent:main:session-/ });

      await expect(async () => {
        await page.getByTestId('sidebar-new-chat').click();
        await expect(globalWorkspaceGroup).toBeVisible({ timeout: 500 });
      ...
      const defaultWorkspaceGroupWithPendingSession = sidebar.locator('[data-testid^="workspace-session-group-"]')
        .filter({ hasText: '默认工作空间' })
        .filter({ hasText: /agent:main:session-/ });
